...
//使用pagemap注册描述符页面，在pagemap中描述符使用的所有页面都将指向desc
//对于（未对齐的）大型分配，只有第一页指向desc
//对齐的大分配获得指向desc的相应页面
...
void* MallocFromActive(ProcHeap* heap)
{
    ...
    } while (!heap->active.compare_exchange_weak(   //compare_exchange_weak CAS无锁函数
            oldActive, newActive));
    ...
    do
    {
        ...
        if (oldCredits == 0)
        {
            ...
            else
            {
                ...
                /*线程检查这次操作是否提取了最后一个credit。如果确实是最后一个(credit==0)，
                那么检查superblock是否有更多可用的block(由于descriptor.maxcount大于MAXCREDITS或者有block刚被free)。
                如果有更多block可用(count>0)，线程将尽可能多地保留block。否则(count=0)，将这个superblock声明为FULL*/
    ...
    //当这个线程提取了credit，它会试着执行UpdateActive函数来更新Active结构。
    if (credits > 0)
...
void UpdateActive(ProcHeap* heap, Descriptor* desc, uint64_t credits)
{
    ...
    //在重新注册之前，其他线程有可能注册了一个新的superblock
    {
        Anchor oldAnchor = desc->anchor.load();
        Anchor newAnchor;
        do
...
void* MallocFromPartial(ProcHeap* heap)
{
    ...
    do
    {
        ...
        newAnchor.count -= credits;
        //如果还有多余的block，就将这个superblock设为active，否则设为FULL
